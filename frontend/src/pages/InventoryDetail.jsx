    ...
    <div style={{ maxWidth: 640 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 20, flexWrap: 'wrap', gap: 12 }}>
        <div>
          ...
          {item.category && <div style={{ fontSize: 13, color: '#b8a99e', marginTop: 2 }}>{item.category}</div>}
        </div>
        <div style={{ display: 'flex', gap: 8 }}>
          {!editMode && <>
            <button onClick={enterEdit} style={outlineBtn('var(--primary)')}>Edit</button>
            <button onClick={() => openForm('add')} style={solidBtn('#6b8f71')}>+ Add Stock</button>
            <button onClick={() => openForm('remove')} style={solidBtn('#c97b7b')}>− Remove</button>
          </>}
          {editMode && <>
      ...
      {editMode && (
        <div style={{ border: '1px solid #e8dfd6', borderRadius: 8, padding: '16px 20px', marginBottom: 24 }}>
          ...
          <div style={{ borderTop: '1px solid #e8dfd6', paddingTop: 10, marginTop: 4, marginBottom: 10 }}>
            <div style={{ fontSize: 12, fontWeight: '600', color: '#b8a99e', marginBottom: 8 }}>UNIT CONVERSION</div>
            <div style={{ display: 'flex', gap: 10, marginBottom: 10 }}>
              <label style={{ flex: 1 }}>
      ...
      {!editMode && (
        <div style={{ display: 'flex', gap: 16, marginBottom: 24, flexWrap: 'wrap' }}>
          <StatBox label="Current Stock" value={displayStock} color={low ? '#c97b7b' : 'var(--primary)'} />
          {showInConversion && (
            <StatBox label={`In ${item.unit || 'base unit'}`} value={`${item.stock_quantity} ${item.unit || ''}`} color="#7a6a5f" />
          )}
          <StatBox label="Low Stock Alert" value={item.low_stock_threshold > 0 ? item.low_stock_threshold : 'Off'} color="#7a6a5f" />
          {hasConversion && (
            <StatBox label="Conversion" value={`1 ${item.conversion_unit} = ${item.conversion_factor} ${item.unit || ''}`} color="#7a5c2e" />
          )}
        </div>
      )}

      {!editMode && low && (
        <div style={{ marginBottom: 20, padding: '10px 16px', background: '#fdf3e3', border: '1px solid #e8dfd6', borderRadius: 8, color: '#c97b7b', fontWeight: '600', fontSize: 13 }}>
      ...
      {/* Stock form */}
      {mode && (
        <div style={{ marginBottom: 24, padding: '16px 20px', border: `1px solid ${mode === 'add' ? '#c8e6ca' : '#e8dfd6'}`, borderRadius: 8, background: mode === 'add' ? '#edf4ee' : '#faeaea' }}>
          <div style={{ fontWeight: '600', marginBottom: 12, color: mode === 'add' ? '#3d5c41' : '#c97b7b' }}>
          ...
          {inputUnit && item.conversion_factor && qty && (
            <div style={{ fontSize: 12, color: '#7a6a5f', marginBottom: 8 }}>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            <button onClick={closeForm} style={outlineBtn('#7a6a5f')}>Cancel</button>
            <button onClick={submitStock} disabled={saving} style={solidBtn(mode === 'add' ? '#6b8f71' : '#c97b7b')}>
              {saving ? '…' : 'Confirm'}
            </button>
          </div>
        </div>
      ...
      {movements.length === 0 ? (
        <p style={{ color: '#c8bdb7', fontSize: 13 }}>No movements recorded.</p>
      ) : (
        <div style={{ border: '1px solid #e8dfd6', borderRadius: 8, overflow: 'hidden' }}>
          <div style={{ display: 'grid', gridTemplateColumns: '60px 80px 1fr 140px', padding: '8px 16px', background: '#fdfaf6', borderBottom: '1px solid #e8dfd6', fontSize: 12, fontWeight: '700', color: '#b8a99e', textTransform: 'uppercase' }}>
            <span>Type</span>
          ...
          {movements.map((m, i) => (
            <div key={m.id} style={{
              ...
              <span style={{
                ...
                background: m.type === 'IN' ? '#edf4ee' : '#faeaea',
                color: m.type === 'IN' ? '#3d5c41' : '#c97b7b',
              }}>{m.type}</span>
              <span style={{ fontWeight: '600', fontSize: 14, color: m.type === 'IN' ? '#6b8f71' : '#c97b7b' }}>
                {m.type === 'IN' ? '+' : '−'}{m.quantity}
              </span>
              <span style={{ fontSize: 13, color: '#7a6a5f' }}>{m.reason || <span style={{ color: '#c8bdb7' }}>—</span>}</span>
              <span style={{ fontSize: 12, color: '#b8a99e', textAlign: 'right' }}>
...
function StatBox({ label, value, color }) {
  return (
    <div style={{ flex: '1 1 140px', padding: '12px 16px', borderRadius: 8, border: `1px solid ${color}22`, background: `${color}0d` }}>
      ...
      <div style={{ fontSize: 12, color: '#7a6a5f', marginTop: 2 }}>{label}</div>
    </div>
  );
}
